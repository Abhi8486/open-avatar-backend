...
class ChatStream:
    ...
    def cancel(self, storage: "StreamStorage"):
        # Cannot cancel a stream that is already CANCELLED
        if self.status == ChatStreamStatus.CANCELLED:
            return False
        # Allow cancelling any stream not in CANCELLED status
        # Even if ENDED with no downstream references, STREAM_CANCEL signal must still be sent
        # because a handler may still be processing this stream (e.g. LLM processing HUMAN_TEXT)
...
class ChatDataSubmitter:
    ...
    def submit(self, data: Union[StreamableData, Tuple[ChatDataType, StreamableData]],
        ...
        data_type = None
        streamers = None
        stream_data = data  # Actual data to be streamed
        ...
        if isinstance(data, ChatData):
            ...
            streamers = self.get_streamers(data_type)
            stream_data = raw_data  # Use extracted raw data
        else:
