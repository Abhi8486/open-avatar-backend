...
class SemanticTurnDetectorHandler(HandlerBase):
    ...
    # Prompt templates
    INTERRUPT_PROMPT = """You are a conversation interrupt detector. A user is having a conversation with a digital avatar, and the avatar is currently speaking.

Recent dialog history:
{dialog_history}

The content the avatar is currently speaking (may be incomplete):
{avatar_text}

What the user just said:
{user_text}

Please judge whether the user wants to interrupt the avatar's speech.

Judgment criteria (ordered by priority):
1. [Interrupt] User explicitly asks to pause, wait, or stop speaking:
   - Chinese: 等一下, 等等, 停, 暂停, 慢点, 别说了
   - English: wait, wait a moment, hold on, stop, pause, one moment, just a second
2. [Interrupt] User responds, asks a question, or negates the current spoken content
3. [Interrupt] User starts talking about a new topic or asks a new question
4. [No Interrupt] User only makes simple filler words/modal particles with no practical content:
   - Chinese: 嗯, 哦, 啊, 好, 好的
   - English: uh, um, uh-huh, hmm

Note: Phrases explicitly expressing wait intent such as "wait a moment", "hold on" must be judged as "打断".

Answer only "打断" or "不打断", with no other content."""

    COMPLETION_PROMPT = """You are a speech completion detector. A user is having a conversation with a digital avatar.

Recent dialog history:
{dialog_history}

What the user currently said (may be incomplete):
{user_text}

Please judge whether the user's speech has fully expressed their intent and is ready for the avatar to reply.

Judgment criteria:
- If sentence structure is complete and meaning is clear, judge as "完成"
- If sentence is obviously incomplete (e.g. "I want to...", "Can you..." but not finished), judge as "未完"
- If it's only filler words or very short words that cannot be judged, judge as "未完"

Answer only "完成" or "未完", with no other content."""
...
"""
    ...
    def _submit_human_text(self, context: SemanticTurnDetectorContext, text: str,
        ...
        # Explicitly specify using only current input stream as the unique source stream
        # This ensures HUMAN_TEXT only associates with one HUMAN_DUPLEX_TEXT, not multiple
        ...
        # Use inputs.stream_id as the unique source stream
        source_streams = [inputs.stream_id] if inputs.stream_id else []
        output_streamer.new_stream(source_streams)

        # Check if stream is auto-cancelled (because parent was cancelled)
        # If so, skip stream_data to avoid triggering automatic collection of all input streams
