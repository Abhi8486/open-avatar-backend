...
class TTSContext(HandlerContext):
    ...
    def handle_text_stream(self, data: ChatData, handler: 'HandlerTTS'):
        ...
        if session is None:
            # New input stream arrived, cancel all old sessions (only one synthesizer active at a time)
            ...
            # Create output stream for new input stream, establishing explicit 1:1 binding
...
class CosyvoiceCallBack(ResultCallback):
    ...
    def _submit_end_frame(self) -> None:
        if self._finished:
            return
        self._finished = True
        # If session is cancelled, do not submit end frame (avoid finishing new stream)
