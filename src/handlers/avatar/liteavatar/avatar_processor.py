...
class AvatarProcessor:
    ...
    def interrupt(self):
        """
        Interrupt current speech: clear all intermediate processing queues
        Pipeline: _audio_slice_queue → _signal_queue → _mouth_img_queue → output
        All queues must be cleared, otherwise residual data will continue to produce audio/video
        """
        ...
        # Reset speech state so subsequent idle frames are in LISTENING status rather than SPEAKING
        self._last_speech_ended = True
        # Reset audio aligner to clear cached audio segments
