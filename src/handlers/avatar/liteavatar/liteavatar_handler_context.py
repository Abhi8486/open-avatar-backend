...
class HandlerTts2FaceContext(HandlerContext):
    ...
    def interrupt(self):
        """Interrupt current speech playback, clear queues and notify worker process"""
        logger.info("LiteAvatar interrupt: clearing output queues and notifying worker")
        # CLIENT_PLAYBACK stream is automatically cancelled by engine's forward_cancel_signal cascade,
        # here we only clean tracking state (preventing _event_out_loop from repeated finish later)
        self._current_tts_stream_key = None
        # Clear audio and video output queues
        ...
        while not self.lite_avatar_worker.video_out_queue.empty():
            try:
                ...
                break
        # Notify worker process to clear audio processing queue
