    ...
    def handle_audio_stream(self, data: ChatData, handler: 'HandlerASR'):
        ...
        try:
            ...
            if callback.error_message:
                ...
                # Close output stream to notify downstream handlers (if active stream exists)
            ...
            output_streamer = self.data_submitter.get_streamer(ChatDataType.HUMAN_TEXT)
            # Explicitly specify using only current session input stream as sole source
            # This ensures 1:1 mapping and avoids linking multiple (including cancelled) input streams
            output_streamer.new_stream([session.input_stream_id])

            # Check if stream was auto-cancelled (because parent was cancelled)
...
class HandlerASR(HandlerBase, ABC):
    ...
    def on_signal(self, context: HandlerContext, signal: ChatSignal):
        """Handle STREAM_CANCEL signal, terminating processing of cancelled stream"""
        context = cast(BailianASRContext, context)
        if signal.type == ChatSignalType.STREAM_CANCEL and signal.related_stream:
            stream_key = signal.related_stream.key
            # If session is still processing, close it
    ...
    def destroy_context(self, context: HandlerContext):
        ...
        for session in context.api_links.values():
            # Session may not be established successfully, handle null and fallback reset
            try:
                session.reset()
            except Exception as e:
