...
class HandlerSmartTurnEOU(HandlerBase, ABC):
    ...
    def get_handler_detail(self, session_context: SessionContext, context: HandlerContext) -> HandlerDetail:
        """Define input/output data types - Bypass monitoring mode: does not consume or produce data."""
        inputs = {
            ChatDataType.HUMAN_AUDIO: HandlerDataInfo(
                type=ChatDataType.HUMAN_AUDIO,
                input_consume_mode=ChatDataConsumeMode.DEFAULT,  # Bypass monitoring, does not block data stream
                input_priority=100,  # Low priority, does not affect ASR
            )
        }
        
        # Does not output data, only emits signals
    ...
    def handle(self, context: HandlerContext, inputs: ChatData, output_definitions: Dict[ChatDataType, HandlerDataInfo]):
        """
        Handle incoming HUMAN_AUDIO data from VAD (Bypass monitoring mode).
        
        Logic:
        1. Accumulate audio data into buffer
        2. Detect EVT_EARLY_VAD_END Event
        3. If detected, run Smart Turn model inference
        4. If judged complete, send candidate STREAM_END signal to VAD
        5. If judged incomplete, wait for next early_vad_end (VAD sends continuously)
        6. Do not output data (bypass monitoring mode)
        """
        ...
        # Detect EVT_EARLY_VAD_END Event
        ...
        if has_early_end and not context.end_signal_sent:
            # Run Smart Turn model inference
            ...
            if buffered_audio is not None and len(buffered_audio) > 0:
                ...
                if prediction == 1:
                    # Judged complete, send candidate STREAM_END signal
                    ...
                    # Judged incomplete, wait for next early_vad_end (VAD sends continuously)
                    logger.info("Smart Turn EOU: Utterance incomplete, waiting for next early_vad_end")
        
        # Handle actual stream end (formal end emitted by VAD)
