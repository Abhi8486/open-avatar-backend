...
def format_approval_notification(info: dict) -> str:
    ...
    return (
        ...
        f"OpenClaw backend requires your approval to execute the following command:\n"
        f"  Command: {info['command']}\n"
        f"  approval_id: {info['approval_id']}\n"
        f"Please briefly inform the user of the command to execute and present the three options:\n"
        f"  1. Allow (single use) → decision=\"allow-once\"\n"
        f"  2. Always allow (no future prompt for similar) → decision=\"allow-always\"\n"
        f"  3. Deny → decision=\"deny\"\n"
        f"If multiple similar approvals occurred in this task, suggest selecting 'Always allow'.\n"
        f"Immediately invoke exec_approve tool upon receiving user response."
    )
